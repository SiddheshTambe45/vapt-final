...
async function runDockerContainer(repoName, branch, fullName) {
  ...
  const dockerCommand = [
    ...
    "-w",
    "/hh", // Set working directory to /hh (repo root)
    "-e",
    `DATABASE_URL=postgresql://postgres.bowwouiwgksgbieqximw:6M1d59uBZAitGshi@aws-0-ap-south-1.pooler.supabase.com:6543/postgres`, // Pass DATABASE_URL environment variable
    "-p",
    "4000:4000", // Bind port 4000 of the container to port 4000 on the host
    "node:18",
    "bash",
    "-c",
    `
      if [ ! -d "/hh/${repoName}/.git" ]; then
        echo "🔄 Cloning repository..."; 
        git clone https://github.com/${fullName}.git /hh/vuln-tang-clan;  # Clone to /hh/vuln-tang-clan
      else 
        cd /hh/${repoName} && echo "🔄 Resetting repo...";
        git reset --hard HEAD && git pull;
      fi;
  
      cd /hh/${repoName};  # Change to the actual repo directory after cloning
      echo "🚀 Checking out branch ${branch}";
      git checkout ${branch};
      ...
      echo "✅ Starting application on port 4000...";
      PORT=4000 npm run build && PORT=4000 npm start;  # Set PORT for both build and start
  ...
  const dockerCommand = [
    ...
    `
      ...
      if git diff --quiet && git diff --cached --quiet; then
        ...
        npm run build;  # Build only if there are changes
      fi;
  
      echo "✅ Starting application on port 4000...";
      PORT=4000 npm start;  # Start the app regardless of build status
...
export const triggerDockerAppSetup = async (payload) => {
  ...
  const targetUrl = "http://localhost:4000"; // Change this to your app's URL if different

  // // Step 2: After app container is set up, run ZAP container
  // console.log("🔹 Starting ZAP container...");
  // await runZapContainer(payload); // Ensure this starts the ZAP container

  // // // Step 3: Start ZAP scan after ZAP container is running
  // // console.log("🔹 Starting ZAP scan...");
  // // await startZapScan(targetUrl); // This starts the security scan

  // // Step 3: Wait until the app is up and running
  // const appReady = await checkAppReady(targetUrl);
  // if (appReady) {
  //   // Step 4: Start ZAP scan after the app is ready
  //   console.log("🔹 Starting ZAP scan...");
  //   await startZapScan(targetUrl); // This starts the security scan
  // } else {
  ...
  // Step 2: Check if app is ready before proceeding
